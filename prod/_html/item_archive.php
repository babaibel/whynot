<!DOCTYPE html>
<html lang="ru">
<?php $pageTitle = 'Мероприятие архивное'; ?>
<?php include 'base/head.php';?>
<body>
	<div class="layout">
		<?php
			$headerInner = 'true';
			$headerCabinet = 'true';
			include 'base/header.php';?>

		<main class="page-body page-body--gray-l">

			<section class="item-page">
				<div class="wr">

					<h1 class="h3-bold-narrow">Катание на воздушных шарах в Московской области</h1>

					<div class="grid-right-fix">
						
						<div class="grid-right-fix__left">
							<div class="grid-right-fix__left-inner">

								<div class="actions-sl actions-sl--inner js-actions-sl2 status-archive">
									<a class="actions-sl__item" href="#!" style="background-image: url(../images/userfiles/slide1.jpg);">
									</a>
									<a class="actions-sl__item" href="#!" style="background-image: url(../images/userfiles/slide1.jpg);">
									</a>
									<a class="actions-sl__item" href="#!" style="background-image: url(../images/userfiles/slide1.jpg);">
									</a>
									<a class="actions-sl__item" href="#!" style="background-image: url(../images/userfiles/slide1.jpg);">
									</a>
								</div>

							</div>
						</div>

						<div class="grid-right-fix__right">
							
							<div class="sidebar sidebar--green">
								<div class="sidebar__item sidebar__item--archive">
									Мероприятие завершено
								</div>
								<div class="sidebar__item">
									<div class="sidebar-text sidebar-text--item">
										<div class="sidebar-text__inner">
											<h3 class="h3-narrow">Когда?</h3>
										</div>
										<div class="sidebar-text__inner">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating rating--sm rating--5">
													<div class="rating__star rating__star1">
														<span class="iconic iconic--star rating__star-inactive"></span>
														<input type="radio" value="1" name="rating">
													</div>
													<div class="rating__star rating__star2">
														<span class="iconic iconic--star rating__star-inactive"></span>
														<input type="radio" value="2" name="rating">
													</div>
													<div class="rating__star rating__star3">
														<span class="iconic iconic--star rating__star-inactive"></span>
														<input type="radio" value="3" name="rating">
													</div>
													<div class="rating__star rating__star4">
														<span class="iconic iconic--star rating__star-inactive"></span>
														<input type="radio" value="4" name="rating">
													</div>
													<div class="rating__star rating__star5">
														<span class="iconic iconic--star rating__star-inactive"></span>
														<input type="radio" value="5" name="rating">
													</div>
													<div class="rating__active-stars">
														<div class="rating__star">
															<span class="iconic iconic--star rating__star-active"></span>
														</div>
														<div class="rating__star">
															<span class="iconic iconic--star rating__star-active"></span>
														</div>
														<div class="rating__star">
															<span class="iconic iconic--star rating__star-active"></span>
														</div>
														<div class="rating__star">
															<span class="iconic iconic--star rating__star-active"></span>
														</div>
														<div class="rating__star">
															<span class="iconic iconic--star rating__star-active"></span>
														</div>
													</div>
												</div>
												13 отзывов
											</div>
										</div>
										<div class="sidebar-text__inner">
											<p class="sidebar-text__title">ВОЗРАСТНОЕ ОГРАНИЧЕНИЕ</p>
											<p class="sidebar-text__subtitle">6 лет+</p>
										</div>
										<div class="sidebar-text__inner">
											<p class="sidebar-text__title">ОРГАНИЗАТОР</p>
											<p class="sidebar-text__p"><a href="tel:+7 495 565-32-13" class="link link-und link-und--green-d">Клуб воздухоплавателей</a></p>
										</div>
									</div>
								</div>
								<div class="sidebar__item">
									<p class="sidebar-text__title sidebar-text__title--price">ЦЕНА</p>
									<p class="sidebar-text__subtitle sidebar-text__subtitle--price"><span>3 000</span> руб. за час/ чел.</p>
								</div>
							</div>

						</div>
					</div>

					<div class="grid-right-fix">
						
						<div class="grid-right-fix__left">
							<div class="grid-right-fix__left-inner">

								<p class="text--big-b">Полет на воздушном шаре с обрядом посвящения в воздухоплаватели и вручением сертификата в клубе «Аэронавтика» со скидкой до 64%</p>
								<p>Романтика воздушных полетов никогда еще не была такой близкой. Посмотрите на свою жизнь с высоты птичьего полета, полюбуйтесь окрестностями, полюбите жизнь еще больше. </p>
								<p>Вас ждет увлекательное воздушное путешествие, во время которого вы сможете полюбоваться красивыми подмосковными пейзажами с высоты птичьего полета, торжественная церемония посвящения в воздухоплаватели с шампанским и подарками, которое завершится вручением сертификата покорителя неба.</p>
								<p>Полет на воздушном шаре — отличный вариант для романтического свидания, во время которого можно будет сколько угодно объясняться в любви на высоте 500 метров. Все шары регулярно проходят тщательную проверку соответствия техническим нормам, поэтому полеты на них абсолютно безопасны.</p>
								<p>Полет проходит при наборе группы от 6 до 16 человек. К полетам допускаются участники от 7 лет в сопровождении родителей. Один купон может использовать один человек. Перед полетом необходимо предъявить распечатанный купон.</p>

								<div class="item-map">
									<div class="item-map__inner">
										<div class="item-map__header">
											<div class="item-map__header-title"><h3 class="h4-bold">Место проведения</h3></div>
											<div class="item-map__header-rout">
												<a href="#!" class="btn btn--white js-item-map-btn">Проложить маршрут</a>
												<div class="item-map__header-search js-item-map-input">
													<form class="js-roadmap-form">
														<div class="static-search">
															<input type="text" class="static-search__input js-roadmap-input" placeholder="Где вы сейчас?">
															<button type="submit" class="static-search__btn"><span class="icon iconic--search"></span></button>
														</div>
													</form>
												</div>
											</div>
										</div>
										<div class="item-map__map-wr" id="roadMap" data-point-lat="55.738337152695" data-point-lon="37.516107100524">

										</div>
										<div class="item-map__info">
											<h2 class="h3-narrow">Контактная информация</h2>
											<div class="item-map__info-row">
												<div class="item-map__info-col">
													<div class="item-map__info-item"><b>Адрес:</b> Москва, Тушинская пойма 35, владение 7</div>
													<div class="item-map__info-item"><b>Название:</b> Аэродром Тушино</div>
												</div>
												<div class="item-map__info-col">
													<div class="item-map__info-item item-map__info-item--nowrap"><b>Сайт:</b> <a href="#!" class="link link-und link-und--green-d">airclub.ru</a></div>
													<div class="item-map__info-item item-map__info-item--nowrap"><b>Телефон:</b> <a href="tel: +7 495 777-77-7" class="link link-und link-und--green-d">+7 495 777-77-7</a></div>
												</div>
												<div class="item-map__info-col">
													<div class="item-map__info-item item-map__info-item--nowrap"><b>Почта:</b> <a href="#!" class="link link-und link-und--green-d">mail@airclub.ru</a></div>
												</div>
											</div>
										</div>
									</div>
								</div>

								<div class="social-share social-share--mrg-all">
									<div class="ya-share2" 
										 data-services="vkontakte,facebook,moimir,gplus,twitter"
										 data-counter=""></div>
								</div>

								<div class="comments-wr">
									<div class="comments__header">
										<div class="comments__header-title">
											<h2 class="h3-bold">Отзывы <sup>35</sup></h2>
										</div>

										<div class="comments__header-right">
											<div class="comments__header-sort">
												<span class="comments__header-sort-txt">
													Сортировать
												</span>
												<div class="selector selector--inline js-selector">
													<span class="selector__option-current js-selector-current">
														 По рейтингу
													</span>
													<ul class="selector__options-list js-selector-options">
														<li class="selector__option js-selector-option">
															По рейтингу
														</li>
														<li class="selector__option js-selector-option">
															По отзывам
														</li>
														<li class="selector__option js-selector-option">
															По дате
														</li>
													</ul>
													<input type="hidden" value="" name="">
												</div>
											</div>

											<div class="comments__header-btn">
												<a class="btn btn--green btn--md js-comment-new-btn" href="#!">Оставить отзыв</a>
											</div>
										</div>
									</div>
									<div class="comments__new js-comment-new">
										<div class="comment">
											<div class="comment__img">
												<img src="../images/userfiles/avatar1.jpg" alt="">
											</div>
											<div class="comment__inner">
												<form>
													<div class="comment__textarea">
														<textarea placeholder="Что вы думаете об этом?" class="textarea"></textarea>
													</div>
													<div class="comment__rating">
														<div class="rating-wrap">

															<div class="rating">

																<div class="rating__star rating__star1">
																	<span class="iconic iconic--star rating__star-inactive"></span>
																</div>

																<div class="rating__star rating__star2">
																	<span class="iconic iconic--star rating__star-inactive"></span>
																</div>

																<div class="rating__star rating__star3">
																	<span class="iconic iconic--star rating__star-inactive"></span>
																</div>

																<div class="rating__star rating__star4">
																	<span class="iconic iconic--star rating__star-inactive"></span>
																</div>

																<div class="rating__star rating__star5">
																	<span class="iconic iconic--star rating__star-inactive"></span>
																</div>

																<div class="rating__active-stars">
																	<div class="rating__star">
																		<span class="iconic iconic--star rating__star-active"></span>
																	</div>
																	<div class="rating__star">
																		<span class="iconic iconic--star rating__star-active"></span>
																	</div>
																	<div class="rating__star">
																		<span class="iconic iconic--star rating__star-active"></span>
																	</div>
																	<div class="rating__star">
																		<span class="iconic iconic--star rating__star-active"></span>
																	</div>
																	<div class="rating__star">
																		<span class="iconic iconic--star rating__star-active"></span>
																	</div>
																</div>

															</div>

														</div>
													</div>

													<div class="comment__btn">
														<button class="btn btn--green" type="submit">отправить</button>
													</div>
												</form>
											</div>
										</div>
									</div>
									<div class="comments__body js-comment-hidden">
										<div class="comment">
											<div class="comment__img">
												<img src="../images/userfiles/avatar1.jpg" alt="">
											</div>
											<div class="comment__inner">
												<div class="comment__title">
													Мне очень понравилось!
												</div>
												<div class="comment__rating">
													<div class="rating-wrap">

														<form class="rating rating--5">

															<div class="rating__star rating__star1">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star2">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star3">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star4">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star5">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__active-stars">
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
															</div>

														</form>

													</div>
												</div>
												<div class="comment__text">
													Это идеальная обзорная экскурсия для ознакомления
												</div>
												<div class="comment__date">
													Мигель  18.02.2015 в 15:14
												</div>
											</div>
										</div>
										<div class="comment">
											<div class="comment__img">
												<img src="../images/userfiles/avatar1.jpg" alt="">
											</div>
											<div class="comment__inner">
												<div class="comment__title">
													Мне очень понравилось!
												</div>
												<div class="comment__rating">
													<div class="rating-wrap">

														<form class="rating rating--5">

															<div class="rating__star rating__star1">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star2">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star3">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star4">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star5">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__active-stars">
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
															</div>

														</form>

													</div>
												</div>
												<div class="comment__text">
													Это идеальная обзорная экскурсия для ознакомления
												</div>
												<div class="comment__date">
													Мигель  18.02.2015 в 15:14
												</div>
											</div>
										</div>
										<div class="comment">
											<div class="comment__img">
												<img src="../images/userfiles/avatar1.jpg" alt="">
											</div>
											<div class="comment__inner">
												<div class="comment__title">
													Какая-то херня...
												</div>
												<div class="comment__rating">
													<div class="rating-wrap">

														<form class="rating rating--3">

															<div class="rating__star rating__star1">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star2">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star3">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star4">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__star rating__star5">
																<span class="iconic iconic--star rating__star-inactive"></span>
															</div>

															<div class="rating__active-stars">
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
																<div class="rating__star">
																	<span class="iconic iconic--star rating__star-active"></span>
																</div>
															</div>

														</form>

													</div>
												</div>
												<div class="comment__text">
													Завершить это знакомство Вы можете подъемом на Эйфелеву башню, вх.билеты на которую я смогу Вам выкупить заранее!
												</div>
												<div class="comment__date">
													Мигель  18.02.2015 в 15:14
												</div>
											</div>
										</div>
									</div>
									<button type="button" class="btn btn--more btn--round btn--white js-show-comment" data-url="queries/comment.php">
										<span class="btn__more-ico"></span>
										<span class="btn__txt">Показать еще</span>
									</button>
								</div>

							</div>
						</div>

						<div class="grid-right-fix__right">
							
							<div class="sidebar">
								<a class="card js-card" href="#!">
									<div class="card__top">
										<div class="card__img" style="background-image: url(../images/userfiles/card1.jpg);">
											<?php include "components/post-favorite.php";?>
											<span class="card__label">
												Выбор редакции
											</span>
										</div>
										<div class="card__title">
											<span class="link-und link-und--green-d">Катание на багги «Экстрим под рев мотора»</span>
										</div>
										<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
									</div>
									<div class="card__bottom">
										<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
										<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
										<div class="card__rating">
											<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
											13 отзывов
										</div>
									</div>
								</a>
							</div>

							<div class="sidebar">
								<a class="card card--full js-card" href="#!">
									<div class="card__top">
										<div class="card__img-full"
											 style="background-image: url(../images/userfiles/card-full1.jpg);">
											 <?php include "components/post-favorite.php";?>
										</div>
										<span class="card__label">
											Выбор редакции
										</span>
										<div class="card__title">
											<span class="link link--lime">
												ЭКСКУРСИЯ
												С ДИГГЕРОМ
												«ТАЙНЫ ПОДЗЕМНОЙ
												МОСКВЫ»
											</span>
										</div>
									</div>
									<div class="card__bottom">
										<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
										<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
										<div class="card__rating">
											<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
											13 отзывов
										</div>
									</div>
								</a>
							</div>

							<div class="sidebar">
								<a class="card js-card" href="#!">
									<div class="card__top">
										<div class="card__img" style="background-image: url(../images/userfiles/card1.jpg);">
											<?php include "components/post-favorite.php";?>
											<span class="card__label">
												Выбор редакции
											</span>
										</div>
										<div class="card__title">
											<span class="link-und link-und--green-d">Катание на багги «Экстрим под рев мотора»</span>
										</div>
										<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
									</div>
									<div class="card__bottom">
										<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
										<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
										<div class="card__rating">
											<div class="rating-static">
												<span class="icon iconic--star"></span>
												<span class="icon iconic--star"></span>
												<span class="icon iconic--star"></span>
												<span class="icon iconic--star"></span>
												<span class="icon iconic--star"></span>
											</div>
											13 отзывов
										</div>
									</div>
								</a>
							</div>

						</div>
					</div>

				</div>
			</section>

			<section class="materials-row js-catalog-tabs">
				<div class="wr">
					<div class="materials-header">
						<div class="materials-header__item">
							<ul class="materials-nav">
								<li class="materials-nav__item">
									<a class="materials-header__item js-mr-cat-btn _active" href="#!">Другие мероприятия</a>
								</li>
							</ul>
						</div>
					</div>
					<div class="materials-wr">
						<div class="materials-wr__tab js-mr-cat-tab material-anim-cont _active">
							<!-- <?php include "blocks/materialsRowLoadOut.php";?> -->
							<div class="materials-grid materials-grid--visible js-cards-hidden">
								<div class="materials-grid__col">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card1.jpg);">
												<?php include "components/post-favorite.php";?>
												<span class="card__label">
													Выбор редакции
												</span>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">Катание на багги «Экстрим под рев мотора»</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>

								<div class="materials-grid__col">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card2.jpg);">
												<?php include "components/post-favorite.php";?>
												<span class="card__label">
													Выбор редакции
												</span>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">
													Посещение Эко хаски Парка в Сокольниках
												</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>

								<div class="materials-grid__col">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card3.jpg);">
												<?php include "components/post-favorite.php";?>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">
													Катание на багги
												</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>

								<div class="materials-grid__col">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card2.jpg);">
												<?php include "components/post-favorite.php";?>
												<span class="card__label">
													Выбор редакции
												</span>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">
													Посещение Эко хаски Парка в Сокольниках
												</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>

								<div class="materials-grid__col _hidden">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card1.jpg);">
												<?php include "components/post-favorite.php";?>
												<span class="card__label">
													Выбор редакции
												</span>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">Катание на багги «Экстрим под рев мотора»</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>

								<div class="materials-grid__col _hidden">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card2.jpg);">
												<?php include "components/post-favorite.php";?>
												<span class="card__label">
													Выбор редакции
												</span>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">
													Посещение Эко хаски Парка в Сокольниках
												</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>

								<div class="materials-grid__col _hidden">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card3.jpg);">
												<?php include "components/post-favorite.php";?>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">
													Катание на багги
												</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>

								<div class="materials-grid__col _hidden">
									<a class="card js-card" href="#!">
										<div class="card__top">
											<div class="card__img" style="background-image: url(../images/userfiles/card2.jpg);">
												<?php include "components/post-favorite.php";?>
												<span class="card__label">
													Выбор редакции
												</span>
											</div>
											<div class="card__title">
												<span class="link-und link-und--green-d">
													Посещение Эко хаски Парка в Сокольниках
												</span>
											</div>
											<div class="card__text">Вам не хватает скорости и острых ощущений? За десять минут вы зарядитесь мощной дозой адреналина и сполна </div>
										</div>
										<div class="card__bottom">
											<div class="card__date"><span class="icon iconic--calendar"></span>16 января, 11:00–19:00</div>
											<div class="card__place"><span class="icon iconic--pin"></span>Клуб CrazyDriver</div>
											<div class="card__rating">
												<div class="rating-static">
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
													<span class="icon iconic--star"></span>
												</div>
												13 отзывов
											</div>
										</div>
									</a>
								</div>
							</div>
							<div class="catalog__footer">
								<button type="button" class="btn btn--more btn--round btn--white js-show-cards">
									<span class="btn__more-ico"></span>
									<span class="btn__txt">Показать еще</span>
								</button>
							</div>
						</div>
					</div>
				</div>
			</section>

		</main>

		<?php include 'base/footer.php'; ?>
		<?php include 'base/popups.php'; ?>
	</div>
	<?php include 'base/scripts.php'; ?>
</body>
</html>
